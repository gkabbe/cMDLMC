...
def numpyprint(atoms, names=None, outfile=None):
	...
	print >> outfile, ""
	for x in atoms:
		print >> outfile, "{:4} {: 20} {: 20} {: 20}".format("H" if x["name"] == "AH" else x["name"], x["pos"][0], x["pos"][1], x["pos"][2])
...
#~ def distance_pbc_nonortho_test(a1_pos, a2_pos, pbc):
	#~ h = pbc.reshape((3,3)).T
	#~ h_inv = np.linalg.inv(h)
	#~ 
	#~ lengths = [(i, np.sqrt(np.dot(v,v))) for i, v in enumerate(pbc.reshape((3,3)))]
	...
	#~ #ipdb.set_trace()
	#~ for i in range(3):
		#~ sdiff[i] = sdiff[i] - int(2*sdiff[i]-1)
		#~ 
	#~ rdiff = np.dot(h, sdiff)
	#~ return rdiff
...
def distance_pbc_nonortho_bruteforce(a1_pos, a2_pos, pbc):
	...
	#~ diffvec = a2_pos - a1_pos
	#~ pbc_proj = [np.dot(diffvec, pbc[i])/np.dot(pbc[i], pbc[i]) for i in range(3)]
	
	#~ diffvec = (pbc_proj[0] % 1) * pbc[0] + (pbc_proj[1] % 1) * pbc[1] + (pbc_proj[2] % 1) * pbc[2]
	#~ for i in range(3):
		#~ while pbc_proj[i] > np.sqrt(np.dot(pbc[i], pbc[i]))/2:
			#~ diffvec -= pbc[i]
			#~ pbc_proj[i] = np.dot(diffvec, pbc[i])/np.sqrt(np.dot(pbc[i], pbc[i]))
		#~ while pbc_proj[i] < -np.sqrt(np.dot(pbc[i], pbc[i]))/2:
			#~ diffvec += pbc[i]
			#~ pbc_proj[i] = np.dot(diffvec, pbc[i])/np.sqrt(np.dot(pbc[i], pbc[i]))
