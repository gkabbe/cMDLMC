...
def parse_frame_xyz_numpy(f):
	...
	for line in f:
		...
		if spl[0] in ACCEPTED_FIRST_COLUMN:
			atoms.append((spl[-1], map(float, spl[5:8])))
...
def parse_frame_full_numpy(f):
	...
	for line in f:
		if line.split()[0] in ACCEPTED_FIRST_COLUMN:
			try:
				atoms.append((line[0:6], line[6:11], line[12:16], line[16], line[17:20], line[21], int(line[22:26]), line[26], map(float,[line[30:38], line[38:46], line[46:54]]),\
...
def write_frame_numpy(f, atoms):
	for atom in atoms:
		print >> f, ATOMFORMAT.format(atom["rec"], atom["serialnr"], atom["name"], atom["locind"], atom["resname"], atom["chainid"], atom["rsn"], atom["rescode"],\
